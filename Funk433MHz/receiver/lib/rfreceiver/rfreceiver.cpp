...
unsigned long RFReceiver::lastTimestamp = 0;
volatile bool RFReceiver::inputBufReady = false;
byte RFReceiver::changeCount = 0;
unsigned int RFReceiver::pulseLimit = 0;
byte RFReceiver::shiftByte = 0;
byte RFReceiver::bitCount;
byte RFReceiver::byteCount;
byte RFReceiver::errorCorBufCount;
bool RFReceiver::packageStarted = false;
byte RFReceiver::inputBufLen = 0;
uint16_t RFReceiver::checksum = 0;
byte RFReceiver::errorCorBuf[3];
byte RFReceiver::inputBuf[MAX_PACKAGE_SIZE];
byte RFReceiver::prevPackageIds[MAX_SENDER_ID + 1];


RFReceiver:: RFReceiver(byte inputPin, unsigned int pulseLength) : inputPin(inputPin) {
    pulseLimit = (pulseLength << 2) - (pulseLength >> 1);
}

void RFReceiver::begin() {
    pinMode(inputPin, INPUT);
    attachInterrupt(inputPin,RFReceiver::handleInterrupt,CHANGE);
}
...
byte RFReceiver::recoverByte(const byte b1, const byte b2, const byte b3) {
...
uint16_t RFReceiver::crc_update(uint16_t crc, uint8_t data) {
    data ^= crc & 0xFF;
    data ^= data << 4;
